...
| 2026-09-26 | Build scripts / worktrees | `scripts/build-*-docker.sh` bind-mount the repo as `/workspace` and run git there. In a `git worktree`, `.git` is a file pointing at a host path the container can't see, so `git config --global` fails, the `safe.directory` exception is never written, and the script tries to re-clone MySQL, then fails (exit 128). | Build from a plain copy (`rsync --exclude .git` plus a `cp -al` of `mysql-server-mysql-<ver>/`, with its `bld-*` removed), not from a worktree. Parallel sessions share `/home/ubuntu/myvector`, so do branch work in a worktree anyway. |
