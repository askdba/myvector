...
if ! docker exec "$CONTAINER" mysql "${MYSQL_ARGS[@]}" -e "SELECT 1"; then
...
if ! docker exec "$CONTAINER" mysql "${MYSQL_ARGS[@]}" -e "SELECT 1" >/dev/null 2>&1; then
	if docker exec "$CONTAINER" mysql -uroot -e "SELECT 1" >/dev/null 2>&1; then
		MYSQL_ARGS=(-uroot)
	else
...
docker exec "$CONTAINER" mysql "${MYSQL_ARGS[@]}" -e "
...
docker exec "$CONTAINER" mysql "${MYSQL_ARGS[@]}" mysql -e "
...
docker exec "$CONTAINER" mysql "${MYSQL_ARGS[@]}" "$DB" -e "
...
BUILD_OUT=$(docker exec "$CONTAINER" mysql "${MYSQL_ARGS[@]}" "$DB" -N -e "
...
SEARCH_OUT=$(docker exec "$CONTAINER" mysql "${MYSQL_ARGS[@]}" "$DB" -N -e "
