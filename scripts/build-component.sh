...
if [ "$OS" = "linux" ]; then
	make -C "$BUILD_DIR" VERBOSE=1 -j"$(nproc)" 2>&1 | tee "$BUILD_LOG"
	MAKE_EXIT=${PIPESTATUS[0]}
else
	make -C "$BUILD_DIR" VERBOSE=1 -j"$(sysctl -n hw.ncpu 2>/dev/null || echo 1)" 2>&1 | tee "$BUILD_LOG"
	MAKE_EXIT=${PIPESTATUS[0]}
fi
...
if [ -f "$BUILD_DIR/libmyvector_component.so" ]; then
	cp "$BUILD_DIR/libmyvector_component.so" "$BUILD_DIR/component/" || {
		echo "Error: failed to copy libmyvector_component.so to component/" >&2
		exit 1
	}
	cp "$REPO_ROOT/src/component_src/myvector.json" "$BUILD_DIR/component/" || {
		echo "Error: failed to copy myvector.json to component/" >&2
		exit 1
	}
	echo "Built: build/libmyvector_component.so, build/component/myvector.json"
elif [ -f "$BUILD_DIR/libmyvector_component.dylib" ]; then
	cp "$BUILD_DIR/libmyvector_component.dylib" "$BUILD_DIR/component/" || {
		echo "Error: failed to copy libmyvector_component.dylib to component/" >&2
		exit 1
	}
	cp "$REPO_ROOT/src/component_src/myvector.json" "$BUILD_DIR/component/" || {
		echo "Error: failed to copy myvector.json to component/" >&2
