...
The build is driven by **CMake**; only `CMakeLists.txt` is committed. The top-level `Makefile` is generated by CMake and is listed in `.gitignore`—do not commit it. Regenerate the build locally:

```bash
# From the repository root (out-of-tree build recommended)
cmake -B build -S .
make -C build
```

For the **standalone component** (e.g. to produce `libmyvector_component.so` / `libmyvector_component.dylib`), use the component script (see [docs/COMPONENT_MIGRATION_PLAN.md](docs/COMPONENT_MIGRATION_PLAN.md)):

```bash
./scripts/build-component.sh [mysql-version]
```
