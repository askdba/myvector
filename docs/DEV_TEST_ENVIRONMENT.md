...
### Already on a typical dev host (confirmed present this session)

| Tool | Version confirmed this session |
|---|---|
| Docker | 29.1.3 |
| cmake | 3.28.3 |
...
```
This succeeded with no sudo password prompt in this session.

**`mysql_config`** is *not* installed on this host, and there is no plan to
install it. This means CLAUDE.md's "Quick build" path (`make`, which
requires `mysql_config` in `PATH`) **does not work here** — only the
Docker-based scripts (`scripts/build-component-*-docker.sh`,
`scripts/build-plugin-8.4-docker.sh`) produce a working `.so` on this kind
of host. Don't chase the `make` path; use the Docker scripts below.
...
| Distribution used by published `ghcr.io/askdba/myvector:mysql8.4` images | No (older images use plugin form; see `docs/COMPONENT_MIGRATION_PLAN.md` for the migration direction) | Yes |
...
(`gh issue view 144` for the full root-cause writeup). The plugin build is
unaffected because it rewrites queries through a completely different,
stable mechanism — the classic Audit Plugin pre-parse hook
(`plugin_audit.h`) in `src/myvector_plugin.cc`.
...
# Component builds (existing scripts) — ~4-5 min each, cold
...
#    server images are server-only and may omit it — needed by the
#    component .so's dynamic link), install it from the MySQL CDN:
...
container: both the DDL annotation and a `WHERE MYVECTOR_IS_ANN(...)` query
returned correct, nearest-neighbor-ordered results. See
`docs/RELIABLE_TEST_ENV_PLAN.md` and the Task 1 write-up it links for the
full transcript.
