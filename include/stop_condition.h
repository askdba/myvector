...
public:
  ...
  void add_point_to_result(labeltype label, const void *datapoint,
  ...
  void remove_point_from_result(labeltype label, const void *datapoint,
                                dist_t dist) override {
    DOCIDTYPE doc_id = space_.get_doc_id(datapoint);
    doc_counter_[doc_id] -= 1;
    if (doc_counter_[doc_id] == 0) {
...
public:
  EpsilonSearchStopCondition(float epsilon, size_t min_num_candidates,
