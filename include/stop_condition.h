...
namespace hnswlib {
    ...
    class MultiVectorInnerProductSpace
        : public BaseMultiVectorSpace<DOCIDTYPE> {
        DISTFUNC<float> fstdistfunc_;
        size_t data_size_;
        size_t vector_size_;
        size_t dim_;
    ...
    public:
        MultiVectorInnerProductSpace(size_t dim) {
            ...
            if (AVX512Capable()) {
                ...
                InnerProductDistanceSIMD16Ext =
                    InnerProductDistanceSIMD16ExtAVX512;
            } else if (AVXCapable()) {
                InnerProductSIMD16Ext = InnerProductSIMD16ExtAVX;
                InnerProductDistanceSIMD16Ext =
                    InnerProductDistanceSIMD16ExtAVX;
            }
#elif defined(USE_AVX)
            if (AVXCapable()) {
                InnerProductSIMD16Ext = InnerProductSIMD16ExtAVX;
    ...
    class MultiVectorSearchStopCondition
        : public BaseSearchStopCondition<dist_t> {
    ...
    public:
        ...
        void add_point_to_result(labeltype label,
                                 const void* datapoint,
                                 dist_t dist) override {
        ...
        void remove_point_from_result(labeltype label,
                                      const void* datapoint,
                                      dist_t dist) override {
            DOCIDTYPE doc_id = space_.get_doc_id(datapoint);
            doc_counter_[doc_id] -= 1;
        ...
        bool should_stop_search(dist_t candidate_dist,
                                dist_t lowerBound) override {
            bool stop_search =
                candidate_dist > lowerBound && curr_num_docs_ == ef_collection_;
            return stop_search;
        }

        bool should_consider_candidate(dist_t candidate_dist,
                                       dist_t lowerBound) override {
        ...
        void filter_results(
            std::vector<std::pair<dist_t, labeltype>>& candidates) override {
    ...
    public:
        ...
        bool should_stop_search(dist_t candidate_dist,
            ...
            if (candidate_dist > lowerBound &&
                curr_num_items_ == max_num_candidates_) {
                // new candidate can't improve found results
                return true;
            }
            if (candidate_dist > epsilon_ &&
                curr_num_items_ >= min_num_candidates_) {
