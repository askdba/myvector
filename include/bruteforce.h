...
namespace hnswlib {
    ...
    public:
        ...
        BruteforceSearch(SpaceInterface<dist_t>* s, size_t maxElements) {
            ...
            if (data_ == nullptr)
                throw std::runtime_error(
                    "Not enough memory: BruteforceSearch failed to allocate "
                    "data");
            cur_element_count = 0;
        }
        ...
        void addPoint(const void* datapoint,
                      labeltype label,
            ...
            int idx;
            {
                ...
                if (search != dict_external_to_internal.end()) {
                    ...
                    if (cur_element_count >= maxelements_) {
                        throw std::runtime_error(
                            "The number of elements exceeds the specified "
                            "limit\n");
                    }
            ...
            memcpy(data_ + size_per_element_ * idx + data_size_,
                   &label,
                   sizeof(labeltype));
        ...
        void removePoint(labeltype cur_external) {
            ...
            labeltype label =
                *((labeltype*)(data_ +
                               size_per_element_ * (cur_element_count - 1) +
                               data_size_));
        ...
        std::priority_queue<std::pair<dist_t, labeltype>> searchKnn(
            ...
            if (cur_element_count == 0)
                return topResults;
            for (int i = 0; i < k; i++) {
                dist_t dist = fstdistfunc_(query_data,
                                           data_ + size_per_element_ * i,
                                           dist_func_param_);
                labeltype label =
            ...
            dist_t lastdist = topResults.empty()
            ...
            for (int i = k; i < cur_element_count; i++) {
                ...
                if (dist <= lastdist) {
                    labeltype label =
                        *((labeltype*)(data_ + size_per_element_ * i +
                                       data_size_));
                    if ((!isIdAllowed) || (*isIdAllowed)(label)) {
                        topResults.emplace(dist, label);
                    }
