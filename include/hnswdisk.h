...
namespace hnswlib {
    ...
    class HierarchicalDiskNSW : public AlgorithmInterface<dist_t> {
    public:
        ...
        mutable std::atomic<size_t> num_deleted_{
            0};  // number of deleted elements
        size_t M_{0};
        size_t maxM_{0};
        size_t maxM0_{0};
        ...
        bool allow_replace_deleted_ =
            false;  // flag to replace deleted elements
...
#include "hnswdisk.i"
        ...
        HierarchicalDiskNSW(SpaceInterface<dist_t>* s,
            ...
            if (M <= 10000) {
                ...
                HNSWERR << "warning: M parameter exceeds 10000 which may lead "
                           "to adverse "
                           "effects."
                        << std::endl;
                HNSWERR << "         Cap to 10000 will be applied for the rest "
                           "of the "
                           "processing."
                        << std::endl;
                M_ = 10000;
            ...
            /// std::cout << "size_links_level0 = " << size_links_level0_ <<
            /// std::endl; std::cout << "data_size_ = " << data_size_ << ",label
            /// size = " << sizeof(labeltype) << std::endl; std::cout <<
            /// "size_data_per_element = "
            /// << size_data_per_element_ << std::endl;
            offsetData_ = size_links_level0_;
            label_offset_ = size_links_level0_ + data_size_;
            /// std::cout << "offsetData_ " << offsetData_ << ", label_offset_ =
            /// " << label_offset_ << std::endl;
            offsetLevel0_ = 0;
            ...
            if (linkLists_ == nullptr)
                throw std::runtime_error(
                    "Not enough memory: HierarchicalDiskNSW failed "
                    "to allocate linklists");
            size_links_per_element_ =
                maxM_ * sizeof(tableint) + sizeof(linklistsizeint);
            /// std::cout << "size_links_per_element_ = " <<
            /// size_links_per_element_ << std::endl;
            mult_ = 1 / log(1.0 * M_);
            /// std::cout << "mult_ = " << mult_ << std::endl;
            revSize_ = 1.0 / mult_;
        }

        ~HierarchicalDiskNSW() { clear(); }
        ...
        inline void setExternalLabel(tableint internal_id,
        ...
        searchBaseLayer(tableint ep_id, const void* data_point, int layer) {
            ...
            if (!isMarkedDeleted(ep_id)) {
                dist_t dist = fstdistfunc_(
                    data_point, getDataByInternalId(ep_id), dist_func_param_);
...
//                    if (candidate_id == 0) continue;
...
#ifdef USE_SSE
                        _mm_prefetch(
                            getDataByInternalId(candidateSet.top().second),
                            _MM_HINT_T0);
#endif
        ...
        searchBaseLayerST(
            ...
            if (bare_bone_search ||
                ...
                if (!bare_bone_search && stop_condition) {
                    stop_condition->add_point_to_result(
                        getExternalLabel(ep_id), ep_data, dist);
                }
            ...
            while (!candidate_set.empty()) {
                std::pair<dist_t, tableint> current_node_pair =
                    candidate_set.top();
...
//                    if (candidate_id == 0) continue;
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag)) {
                        ...
                        char* currObj1 = (getDataByInternalId(candidate_id));
                        dist_t dist = fstdistfunc_(
                            data_point, currObj1, dist_func_param_);
        ...
        tableint mutuallyConnectNewElement(
            ...
            // MyVector HNSW Recovery - record this new/updated Node
            addNodeToFlushList(cur_c);

            std::set<tableint> linksSet1, linksSet2;

            for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
                std::unique_lock<std::mutex> lock(
                    link_list_locks_[selectedNeighbors[idx]]);

                tableint neighbourId = selectedNeighbors[idx];  // MyVector
                ...
                // `selectedNeighbors[idx]` then no need to modify any
                ...
                if (!is_cur_c_present) {
                    if (sz_link_list_other < Mcurmax) {
                        ...
                        // finding the "weakest" element to replace it with the
                        // new one
                        ...
                        getDataByInternalId(rez[idx]), dist_func_param_); if (d
                    ...
                    // MyVector HNSW Recovery - record this link updated node
                    if (level == 0)
                        // addNodeLinksLevel0ToFlushList(neighbourId);
                        linksSet1.insert(neighbourId);
                    else
        ...
        void resizeIndex(size_t new_max_elements) {
            ...
            if (data_level0_memory_new == nullptr)
                throw std::runtime_error(
                    "Not enough memory: resizeIndex failed to allocate base "
                    "layer");
        ...
        /*
         * Marks an element with the given label deleted, does NOT really change
         * the current graph.
         */
        ...
        /*
         * Uses the last 16 bits of the memory for the linked list size to store
         * the mark, whereas maxM0_ has to be limited to the lower 16 bits,
         * however, still large enough in almost all cases.
         */
        ...
        /*
         * Removes the deleted mark of the node, does NOT really change the
         * current graph.
         *
         * Note: the method is not safe to use when replacement of deleted
         * elements is enabled, because elements marked as deleted can be
         * completely removed by addPoint
         */
        ...
        /*
         * Checks the first 16 bits of the memory to see if the element is
         * marked deleted.
         */
        ...
        void addPoint(const void* data_point,
            ...
            if ((allow_replace_deleted_ == false) &&
                (replace_deleted == true)) {
                throw std::runtime_error(
                    "Replacement of deleted elements is disabled in "
                    "constructor");
            }
            ...
            if (!is_vacant_place) {
                ...
                // element
                labeltype label_replaced =
                    getExternalLabel(internal_id_replaced);
        ...
        void updatePoint(const void* dataPoint,
                         tableint internalId,
            ...
            // If point to be updated is entry point and graph just contains
            // single element then just return.
            ...
            repairConnectionsForUpdate(
                dataPoint, entryPointCopy, internalId, elemLevel, maxLevelCopy);
        }
        ...
        void repairConnectionsForUpdate(const void* dataPoint,
                ...
                for (int level = maxLevel; level > dataPointLevel; level--) {
                    ...
                    while (changed) {
                        ...
                        for (int i = 0; i < size; i++) {
                            ...
                            dist_t d = fstdistfunc_(dataPoint,
                                                    getDataByInternalId(cand),
                                                    dist_func_param_);
                            if (d < curdist) {
                                curdist = d;
            ...
            if (dataPointLevel > maxLevel)
                throw std::runtime_error(
                    "Level of item to be updated cannot be bigger than max "
                    "level");
            ...
            for (int level = dataPointLevel; level >= 0; level--) {
                ...
                // there could be cases where `topCandidates` could just
                // contains entry point itself. To prevent self loops, the
                ...
                if (filteredTopCandidates.size() > 0) {
                    ...
                    currObj = mutuallyConnectNewElement(dataPoint,
                                                        dataPointInternalId,
                                                        filteredTopCandidates,
                                                        level,
                                                        true);
        ...
        tableint addPoint(const void* data_point, labeltype label, int level) {
                ...
                if (search != label_lookup_.end()) {
                    ...
                    if (allow_replace_deleted_) {
                        if (isMarkedDeleted(existingInternalId)) {
                            throw std::runtime_error(
                                "Can't use addPoint to update deleted elements "
                                "if replacement "
            ...
            if (curlevel) {
                ...
                if (linkLists_[cur_c] == nullptr)
                    throw std::runtime_error(
                        "Not enough memory: addPoint failed to allocate "
                        "linklist");
                memset(linkLists_[cur_c],
                       0,
            ...
            if ((signed)currObj != -1) {
                if (curlevel < maxlevelcopy) {
                    dist_t curdist = fstdistfunc_(data_point,
        ...
        std::vector<std::pair<dist_t, labeltype>> searchStopConditionClosest(
            ...
            std::priority_queue<std::pair<dist_t, tableint>,
                ...
                top_candidates;
            top_candidates = searchBaseLayerST<false>(
                currObj, query_data, 0, isIdAllowed, &stop_condition);
