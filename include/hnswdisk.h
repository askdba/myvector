...
#include "hnswdisk.i"
  ...
  HierarchicalDiskNSW(SpaceInterface<dist_t> *s, const std::string &location,
  ...
  HierarchicalDiskNSW(SpaceInterface<dist_t> *s, size_t max_elements,
    ...
    /// std::cout << "data_size_ = " << data_size_ << ",label size = " <<
    /// sizeof(labeltype) << std::endl; std::cout << "size_data_per_element = "
    ...
    if (linkLists_ == nullptr)
      throw std::runtime_error("Not enough memory: HierarchicalDiskNSW failed "
                               "to allocate linklists");
    size_links_per_element_ =
        maxM_ * sizeof(tableint) + sizeof(linklistsizeint);
    /// std::cout << "size_links_per_element_ = " << size_links_per_element_ <<
    /// std::endl;
    mult_ = 1 / log(1.0 * M_);
