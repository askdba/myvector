...
double computeL2Distance(const FP32* __restrict v1,
                         const FP32* __restrict v2,
                         int dim) {
...
double computeIPDistance(const FP32* __restrict v1,
                         const FP32* __restrict v2,
                         int dim) {
...
float computeCosineDistanceFn(const void* __restrict v1,
    ...
    return (float)computeCosineDistance(
        (const FP32*)v1, (const FP32*)v2, (int)qty);
}
...
public:
    HammingBinaryVectorSpace(size_t dim) {
        ...
        dim_ = dim;
        data_size_ = (dim / BITS_PER_BYTE);  /// 1 bit per dimension - assumes
                                             /// dim is multiple of 8
    }
...
bool HNSWMemoryIndex::initIndex() {
    ...
    m_alg_hnsw = new hnswlib::HierarchicalDiskNSW<FP32>(
        m_space, m_size, m_M, m_ef_construction);
...
inline void ParallelFor(size_t start,
                        size_t end,
                        size_t numThreads,
                        Function fn) {
...
bool HNSWMemoryIndex::flushBatchParallel() {
    ...
    ParallelFor(
        0, m_batchkeys.size(), m_threads, [&](size_t row, size_t threadId) {
            m_alg_hnsw->addPoint(
                (void*)&(m_batch[row * m_space->get_data_size()]),
                m_batchkeys[row]);
        });
...
bool HNSWMemoryIndex::insertVector(VectorPtr vec, int dim, KeyTypeInteger id) {
    ...
    if (m_isParallelBuild) {
        ...
        m_batch.insert(
            m_batch.end(), (char*)vec, ((char*)vec + m_space->get_data_size()));
...
bool rewriteMyVectorColumnDef(const string& query, string& newQuery) {
    ...
    while ((pos = newQuery.find(MYVECTOR_COLUMN_A)) != string::npos) {
        ...
        string newColumn = "";
#if MYSQL_VERSION_ID >= 90000
        newColumn = "VECTOR(" + to_string(dim) +
...
bool myvector_query_rewrite(const string& query, string* rewritten_query) {
    ...
    *rewritten_query = query;
    ...
    if ((regex_search(query, select_stmt) ||
         regex_search(query, explain_stmt))) {
...
PLUGIN_EXPORT bool myvector_ann_set_init(UDF_INIT* initid,
                                         UDF_ARGS* args,
                                         char* message) {
    initid->ptr = nullptr;
...
PLUGIN_EXPORT char* myvector_ann_set(UDF_INIT* initid,
                                     UDF_ARGS* args,
                                     char* result,
...
PLUGIN_EXPORT bool myvector_construct_init(UDF_INIT* initid,
                                           UDF_ARGS* args,
                                           char* message) {
...
PLUGIN_EXPORT char* myvector_display(UDF_INIT* initid,
                                     UDF_ARGS* args,
                                     char* result,
                                     unsigned long* length,
                                     char* is_null,
...
PLUGIN_EXPORT bool myvector_distance_init(UDF_INIT* initid,
                                          UDF_ARGS* args,
                                          char* message) {
...
PLUGIN_EXPORT double myvector_hamming_distance(UDF_INIT*,
                                               UDF_ARGS* args,
                                               char* is_null,
                                               char*) {
    double dist = 0.0;
...
void myvector_open_index_impl(char* vecid,
    ...
    /* Admin operations usecases :-
     ...
     5. If tracking column technique -> call myvector("refresh") - index should
     be loaded first. refresh will not persist. After reboot/restart, call
    ...
    */
    ...
    if (!strcmp(action, "build") || !strcmp(action, "refresh")) {
        ...
        *veccol = 0;
        veccol++;
        BuildMyVectorIndexSQL(db,
                              table,
                              pkidcol,
                              veccol,
                              action,
                              trackingColumn.c_str(),
                              vi,
                              errorbuf);
...
PLUGIN_EXPORT char* myvector_search_save_udf(UDF_INIT*,
                                             UDF_ARGS* args,
                                             char* result,
                                             unsigned long* length,
                                             unsigned char* is_null,
...
PLUGIN_EXPORT bool myvector_is_valid_init(UDF_INIT* initid,
                                          UDF_ARGS* args,
                                          char* message) {
    if (!initid || args->arg_count != 2) {
        strcpy(message,
               "Incorrect arguments to myvector_is_valid(), "
...
PLUGIN_EXPORT long long myvector_is_valid(UDF_INIT*,
                                          UDF_ARGS* args,
                                          unsigned char*,
                                          unsigned char*) {
...
PLUGIN_EXPORT bool myvector_row_distance_init(UDF_INIT* initid,
                                              UDF_ARGS* args,
                                              char* message) {
    if (!initid || args->arg_count != 1) {
        strcpy(message,
               "Incorrect arguments to myvector_row_distance(), "
...
PLUGIN_EXPORT double myvector_row_distance(UDF_INIT*,
                                           UDF_ARGS* args,
                                           char*,
                                           char*) {
...
bool isAfter(const string& binlogfile2,
             const size_t binlogpos2,
             const string& binlogfile1,
             const size_t binlogpos1) {
 ...
 */
void myvector_checkpoint_index(const string& dbtable,
                               const string& veccol,
                               const string& binlogFile,
                               size_t binlogPos) {
